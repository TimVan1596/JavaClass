...
<div align='center'>
    <form action='./revokeSearch.jsp' method='post'>
    ...
    <table border="1" width="750" align = "center" style="background-color: white">
        ...
        <%
            ...
            if(request.getParameter("search") == null){
                ...
                //遍历结果集
                num = jdbcUtilDvd.revokeFindCountPage(request.getParameter("search"));
            }
            ...
            if(request.getAttribute("page") == null){
                page1 = 1;
                dvds = jdbcUtilDvd.revokeFindPage(1,request.getParameter("search"));
            }else{
                page1 = (Integer) request.getAttribute("page");
                dvds = jdbcUtilDvd.revokeFindPage(page1,request.getParameter("search"));
            }
            for (Dvd dvd : dvds) {
        %>
        <tr>
        ...
        <tr>
            ...
            <td colspan = '7' align="right">
                <a href = '../../atbrevokePaging.do?page=1&search=<%=request.getParameter("search")%>'>首页</a>
                <%
                    if(page1 == 1){
                %>
                <a href = '../../atbrevokePaging.do?page=1&search=<%=request.getParameter("search")%>'>上一页</a>
                <%
                }else{
                %>
                <a href = '../../atbrevokePaging.do?page=<%= page1-1%>&search=<%=request.getParameter("search")%>'>上一页</a>
                <%
                    }
                %>
                <%
                ...
                %>
                <a href = '../../atbrevokePaging.do?page=<%= i %>&search=<%=request.getParameter("search")%>'><%= i %></a>
                <%
                    }
                %>
                <%
                    if(page1 == pages){
                %>
                <a href = '../../atbrevokePaging.do?page=<%= pages%>&search=<%=request.getParameter("search")%>'>下一页</a>
                <%
                }else{
                %>
                <a href = '../../atbrevokePaging.do?page=<%= page1+1%>&search=<%=request.getParameter("search")%>'>下一页</a>
                <%
                    }
                %>
                <a href = '../../atbrevokePaging.do?page=<%= pages%>&search=<%=request.getParameter("search")%>'>尾页</a>
            </td>
        </tr>
