...
<h5 align='center' style="color: red">界面操作：归还（还一本书）、恢复（恢复数据）、分页（数据分页）、清除（多删要密码无法找回）、借出（如有未还会红色）</h5>
<div align='center'>
    ...
    <table border="1" width="750" align = "center" style="background-color: white">
        ...
        <%
            ...
            if(request.getAttribute("page") == null){
                page1 = 1;
                dvds = jdbcUtilDvd.revokefind(1);
            }else{
                page1 = (Integer) request.getAttribute("page");
                dvds = jdbcUtilDvd.revokefind(page1);
            }
            for (Dvd dvd : dvds) {
        %>
        <tr>
        ...
        <tr>
            ...
            <td colspan = '7' align="right">
                <a href = './atbrevoke.do?page=1'>首页</a>
                <%
                    if(page1 == 1){
                %>
                <a href = './atbrevoke.do?page=1'>上一页</a>
                <%
                }else{
                %>
                <a href = './atbrevoke.do?page=<%= page1-1%>'>上一页</a>
                <%
                    }
                %>
                <%
                ...
                %>
                <a href = './atbrevoke.do?page=<%= i %>'><%= i %></a>
                <%
                    }
                %>
                <%
                    if(page1 == pages){
                %>
                <a href = './atbrevoke.do?page=<%= pages%>'>下一页</a>
                <%
                }else{
                %>
                <a href = './atbrevoke.do?page=<%= page1+1%>'>下一页</a>
                <%
                    }
                %>
                <a href = './atbrevoke.do?page=<%= pages%>'>尾页</a>
            </td>
        </tr>
